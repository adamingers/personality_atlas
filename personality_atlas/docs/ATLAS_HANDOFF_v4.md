...
**Layout (changed)**: the site serves from **`personality_atlas/` in the repo root**, published at `adamshiva.com/personality_atlas/`. `index.html` is a landing page linking to `cooked.html` and `harvested.html` beside it; each chart carries a `⌂ home` link back. All links are relative, so the folder can be renamed or moved freely. The old nested `personalitygraph1_package/personalitygraph1/` path is retired.
...
| Traits | **779** |
| Adjectives | **926** |
| Distinct systems (bodies of work) | **78** |
| Theorists / traditions (grouped) | **67** |
| Convergence clusters | 86 |
| Masters | `atlas_cooked_v13.json`, `atlas_harvested9.json` |
...
The site lives in **`personality_atlas/`** at the repo root and is served at `adamshiva.com/personality_atlas/`.

```
CNAME                       repo ROOT — adamshiva.com. MUST stay at the root.
                            Putting it inside personality_atlas/ breaks the custom domain.
.nojekyll                   repo ROOT — stops Pages preprocessing
personality_atlas/
  index.html                landing page (#cooked / #harvested deep-link straight in)
  cooked.html
  harvested.html
  masters/ build_tools/ docs/    reference only, not part of the UI
```

Publishing = replacing `index.html`, `cooked.html`, `harvested.html` inside `personality_atlas/`. The landing page states the counts (779 / 926 / 66) in its header — update when they change. Pages settings: **Deploy from branch → main → / (root)**; the subfolder is served automatically at its own path.

Because every link is relative, the same folder works unchanged if opened locally, moved to another path, or promoted to the site root later.
...
## 10. Refinement round (this session)

**Data corrections**
- **BFAS was over-attributed.** BFAS has exactly 10 aspects; 37 traits carried its label. The 18 that are not aspects (Achievement-Striving, Aesthetic Sensitivity, Altruism, Artistic Interests, Dutifulness, Imagination, Intellectual Curiosity, Introspection, Liberalism and their low poles) moved to a new system, **IPIP-NEO Facets (Goldberg, 1999)**. BFAS is now 19 (10 aspects + 9 low poles). Verified NEO (30 canonical facets + 24 low poles = 54) and HEXACO (24 facets + 22 low poles = 46) are correct as they stand. NEO is missing low poles for Impulsiveness, Warmth, Fantasy, Trust, Modesty, Order; HEXACO is missing its interstitial Altruism facet.
- Every trait gained **`yr`** (publication year of its system; hand-assigned for the 50 systems whose label carries no date, ranging -800 to 2016) and **`fld`** (one of six consolidated fields). `sys_years` and `field_colors` are stored on the master.
- All **86 convergence clusters are named** from their top three shared adjectives (`conv_meta[i].name`), e.g. *Compassionate · Nurturing · Caring* (39 traits, 27 systems).
- **`short_labels`** maps trait id → a shortened map label for 344 traits (mean 31 → 13 chars). Rule: for an em-dash name, keep whichever half is unique within its own system — so `Life Path 5 — The Adventurer` becomes **Life Path 5** but `Extraversion — Sociability` becomes **Sociability**. Trailing parentheticals dropped when unambiguous; slash alternatives keep the first; reverted where two short labels would land within 220 units. Full names remain everywhere except the map label.
- Harvested gained **`valleys_b1`** and **`valleys_b2`** — two finer watershed tiers computed by sub-splitting each community (2-way, then 4-way) and keeping only boundaries the coarser tier does not already draw.

**Interface**
- **Grid** (`⊞ grid`): 250-unit cells on both charts, labelled `C-A1` / `H-A1` so a reference is unambiguous across maps, with a corner readout of the current cell and visible range.
- **Field colouring** (`◑ field`): hue per field, lightness by era (pale = earlier, deep = later), with its own key. Mutually exclusive with convergence colouring.
- **Convergence browser**: all 86 clusters listed in the Guide tab, tap to light; the region header shows the cluster name so you can tell clusters apart while comparing.
- **Adjective relatedness**: a word's traits are ranked by mean similarity to each other and banded **core / adjacent / outlier**, with an A–Z toggle and a "lasso these N" button.
- **Compare**: seeds from the last two traits viewed instead of an empty state; adjectives are first-class entities (definition, usage share, nearest words, ranked traits) sent to either side by the `⇆` on any adjective pill; listed traits are bordered by their relation to the other side (both / adjacent / distant).
- **Mobile key fix**: the key sits inside the map element, so taps on it were reaching the chart's hit-test. Pointer events are now stopped at the key and intro panel — but note that guarding `click` as well breaks their own close buttons, which is what made the first attempt look identical to the original bug.
- **Labels**: short labels plus a short leader line from each label to its node, which removes the "nearest label belongs to a different circle" ambiguity.

**Build note**: the pristine chart shells were lost in a container reset, so `patch.py` now swaps the feature block inside an already-built chart (from the `/* ===== Atlas extensions` marker to the closing `</script>`) rather than re-running `inject.py` from scratch. Keep a pristine shell if you want to return to the original flow.
